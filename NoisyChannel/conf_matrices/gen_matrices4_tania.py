...
delet = defaultdict(int)
insert = defaultdict(int)
subst = defaultdict(int)
transpos = defaultdict(int)
        ...
        while i < len(correct)-1 and j < len(typo)-1:
            if correct[i] != typo[j]:
                if correct[i+1] == typo[j] and correct[i+2] == typo[j+1]:
                    ...
                    transpos[(correct[i:i + 2], typo[j:j + 2])] += 1
                    i += 2
                    j += 2
                else:
...
if __name__ == '__main__':
    ...
    for name, dct in matrices_dct.items():
        file_name = "conf_matrices/" + name + "_tania.txt"
